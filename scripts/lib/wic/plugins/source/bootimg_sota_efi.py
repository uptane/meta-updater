...
# Copyright (c) 2025, Foundries.io Ltd.
#
...
#
# Heavily based on the bootimg_efi plugin from OE-core, removing logic which
# copies over initrd, kernel and which also sets the default boot
# configuration (handled by ostree).
...
class BootimgSotaEFIPlugin(SourcePlugin):
    ...
    @classmethod
    def do_configure_systemdboot(cls, part, creator, cr_workdir):
        """
        Create loader-specific systemd-boot/gummiboot config
        """
        if part.rootfs_dir is None:
            raise WicError("Couldn't find --rootfs-dir, exiting")
        copyhardlinktree(part.rootfs_dir, cr_workdir + "/hdd")

        install_cmd = "install -d %s/hdd/boot/EFI/systemd" % cr_workdir
        exec_cmd(install_cmd)

        install_cmd = "install -d %s/hdd/boot/loader" % cr_workdir
        exec_cmd(install_cmd)
    ...
    def do_configure_partition(cls, part, source_params, creator, cr_workdir,
        ...
        try:
            if source_params['loader'] == 'grub-efi':
                ...
                cls.do_configure_systemdboot(part, creator, cr_workdir)
            else:
